# Enhanced version of app.py with order tracking, additional visualizations, and trend analysis
import sys
import numpy as np
...
def load_data():
    ...
    alerts_df = inv[inv["stock_level"] <= inv["reorder_point"]]
    orders = pd.read_csv("data/orders.csv")
    sales_history = pd.read_csv("data/sales_history.csv")
    forecast = pd.read_csv("data/forecast.csv")
    return inv, alerts_df, orders, sales_history, forecast

inventory, alerts, orders, sales_history, forecast = load_data()

# Calculate time to stockout based on forecast
def calculate_time_to_stockout(inventory_df, forecast_df):
    # Calculate average daily demand from forecast
    avg_daily_demand = forecast_df['yhat'].mean()
    
    # Calculate days to stockout for each product
    inventory_df['avg_daily_demand'] = inventory_df['product_id'].map(
        lambda x: avg_daily_demand * 0.9 + np.random.normal(0, 5))  # Simulate product-specific demand
    inventory_df['days_to_stockout'] = inventory_df.apply(
        lambda row: round(max(0, row['stock_level'] / max(1, row['avg_daily_demand'])), 1)
        if row['avg_daily_demand'] > 0 else float('inf'), axis=1)
...
# Calculate KPIs
inventory = calculate_time_to_stockout(inventory, forecast)
stockout_trends = calculate_stockout_trends()
...
# New callback to initialize and update the metrics
@app.callback(
...
def update_metrics(warehouse, product):
    ...
    # Calculate low stock counts based on filtered data
    filtered_alerts = filtered_inventory[filtered_inventory["stock_level"] <= filtered_inventory["reorder_point"]]
    
    return str(len(filtered_alerts)), str(len(filtered_inventory))
...
def add_item(n_clicks, pid, name, stock, reorder, warehouse):
    ...
    inventory = calculate_time_to_stockout(inventory, forecast)
    alerts = inventory[inventory["stock_level"] <= inventory["reorder_point"]]
    inventory.to_csv("data/inventory.csv", index=False)
    alerts.to_csv("data/inventory_alerts.csv", index=False)

    return (
        dbc.Alert(f"✅ Product '{name}' added!", color="success"),
...
def update_tabs(tab, warehouse, product):
    ...
    filtered_alerts = df[df["stock_level"] <= df["reorder_point"]]

    # Insights logic
    critical_items = df[df["days_to_stockout"] < 7]
    if len(critical_items) > 0:
        insight = f"\u26A0\uFE0F {len(critical_items)} product(s) will stock out within 7 days! Urgent reorder needed."
    ...
    if tab == "inventory":
        ...
        return dbc.Row([
            dbc.Col([
                dbc.Alert("✅ No alerts!", color="success") if filtered_alerts.empty else
                html.Div([
                    html.H4("Low Stock Alerts", className="text-danger"),
                    dash_table.DataTable(
                        data=filtered_alerts.to_dict('records'),
                        columns=[
                        ...
                        style_data_conditional=[
                            ...
                            {
                                'if': {'filter_query': '{days_to_stockout} < 7'},
        ...
        # Create heatmap data for inventory levels across warehouses and products
        if len(df) > 1:
